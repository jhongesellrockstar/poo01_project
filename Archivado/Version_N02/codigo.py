...
class PetApp(tk.Tk):
    ...
    def cargar_lista_perdidos(self):
        ...
        for m in data:
            ...
            img = self.crear_imagen(m.get('FotoPath'), size=(120, 120), fallback_color="#EEEEEE")
            tk.Label(tk.Frame(card, bg="white", width=120, height=120), image=img, bg="white").pack(side="left", padx=(0, 20))
            info = tk.Frame(card, bg="white"); info.pack(side="left", fill="both", expand=True)
            tk.Label(info, text=m['Nombre'] or "Sin nombre", font=("Arial", 18, "bold"), fg="#333", bg="white").pack(anchor="w")
            tk.Label(info, text=m.get('Descripcion', ''), font=("Arial", 11), fg="#555", bg="white").pack(anchor="w")
            tk.Button(card, text="📍 VER UBICACIÓN", bg=COLOR_SECONDARY, fg="white", font=("Arial", 10, "bold"), padx=15, pady=10, bd=0, cursor="hand2", command=lambda x=m: self.ver_mapa_perdido(x)).pack(side="right", padx=20)
