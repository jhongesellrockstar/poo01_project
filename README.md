...
COLOR_BG_MAIN = "#FDF6E4"   # Crema suave
...
class PetApp(tk.Tk):
    ...
    # NUEVO: Generador de Icono de Éxito (Corazón)
    def crear_icono_exito(self):
        size = (120, 120)
        # Fondo transparente (usamos el color de fondo de la app para simularlo)
        img = Image.new('RGB', size, color=COLOR_BG_MAIN) 
        d = ImageDraw.Draw(img)
        # Dibujar un círculo verde
        d.ellipse([10, 10, 110, 110], fill=COLOR_ACCENT)
        # Dibujar un corazón o texto simple en medio
        # Como no podemos cargar fuentes externas facilmente, dibujaremos un "+" simple o check
        d.line([(40, 60), (55, 80)], fill="white", width=8) # Check parte 1
        d.line([(55, 80), (85, 40)], fill="white", width=8) # Check parte 2
        return ImageTk.PhotoImage(img)
    ...
    def mostrar_adopcion(self):
        ...
        lbl_style = {"bg": COLOR_BG_MAIN, "fg": COLOR_TEXT, "font": ("Helvetica", 10, "bold")}
        tk.Label(filter_frame, text="Tipo:", **lbl_style).pack(side="left", padx=(0,5))
        ...
        tk.Label(filter_frame, text="Etapa:", **lbl_style).pack(side="left", padx=(20,5))
        ...
        def renderizar_lista():
            ...
            for animal in self.adopcion_data:
                ...
                if match_tipo and match_edad:
                    ...
                    det_frame = tk.Frame(card, bg="white", pady=5)
                    det_frame.pack()
                    tk.Label(det_frame, text=f"🐾 {animal['raza']}", font=FONT_BODY, bg="white", fg=COLOR_TEXT_LIGHT).pack()
                    tk.Label(det_frame, text=f"🎂 {animal['edad']} | {animal['sexo']}", font=FONT_BODY, bg="white", fg=COLOR_TEXT_LIGHT).pack()
    ...
    def mostrar_formulario_adopcion(self, animal):
        ...
        tk.Label(input_frame, text="Nombre Completo:", bg=COLOR_BG_CARD, font=FONT_BODY, fg=COLOR_TEXT).grid(row=0, column=0, sticky="e", pady=10, padx=10)
        ...
        tk.Label(input_frame, text="Celular:", bg=COLOR_BG_CARD, font=FONT_BODY, fg=COLOR_TEXT).grid(row=1, column=0, sticky="e", pady=10, padx=10)
        ...
        # --- CAMBIO: Función Enviar ahora redirige a la pantalla de gracias ---
        def enviar():
            nombre = entry_nombre.get()
            contacto = entry_cel.get()
            if nombre and contacto:
                # LLAMAR A LA NUEVA PANTALLA
                self.mostrar_agradecimiento(nombre, animal['nombre'])
            else:
    ...
    # ==========================================
    # PANTALLA 3.5: AGRADECIMIENTO (NUEVA)
    # ==========================================
    def mostrar_agradecimiento(self, nombre_usuario, nombre_mascota):
        ...
        card.place(relx=0.5, rely=0.5, anchor="center") # Centrado perfecto

        # Icono de éxito (generado con PIL)
        icon_img = self.crear_icono_exito()
        lbl_icon = tk.Label(card, image=icon_img, bg="white")
        lbl_icon.image = icon_img # Referencia
        ...
        tk.Label(card, text=f"Hemos registrado tu interés por {nombre_mascota}.", 
                 font=FONT_BODY, fg=COLOR_TEXT_LIGHT, bg="white").pack(pady=(10, 5))
        
        tk.Label(card, text="Nos pondremos en contacto contigo a la brevedad.", 
                 font=FONT_BODY, fg=COLOR_TEXT_LIGHT, bg="white").pack()

        # Botón para volver al inicio
        tk.Button(card, text="Volver al Inicio", bg=COLOR_PRIMARY, fg="white", font=FONT_BUTTON, bd=0, 
                  cursor="hand2", width=20, pady=10, 
                  command=self.mostrar_bienvenida).pack(pady=(40, 0))
