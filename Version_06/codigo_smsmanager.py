...
import random
import pyodbc
import requests  # Para hacer llamadas HTTP a SMSManager
from config import DATABASE_CONFIG, SMSMANAGER_CONFIG
...
class PetApp(tk.Tk):
    def __init__(self):
        ...
        try:
            ...
            # Si hay un error al cargar la base de datos, inicializamos las listas vacías
        ...
        self.db_adopcion = [
            # --- PERROS ---
            {
                "Nombre": "Max", "Tipo": "Perro", "Raza": "Pastor Alemán", "Edad": "Adulto",
                "Descripcion": "Leal y protector. Ideal para casas grandes con jardín.",
                "FotoPath": "imagenes/perros/adultos/imagen1.png"
            },
            {
                "Nombre": "Bella", "Tipo": "Perro", "Raza": "Golden Retriever", "Edad": "Adulto",
                "Descripcion": "Muy cariñosa y tranquila. Se lleva excelente con niños.",
                "FotoPath": "imagenes/perros/adultos/imagen2.png"
            },
            # --- GATOS ---
            {
                "Nombre": "Pelusa", "Tipo": "Gato", "Raza": "Mestizo", "Edad": "Cachorro",
                "Descripcion": "Pequeña bolita de pelo muy juguetona. Necesita mucha atención.",
                "FotoPath": "imagenes/gatos/cachorros/imagen3.png"
            },
            {
                "Nombre": "Salem", "Tipo": "Gato", "Raza": "Bombay", "Edad": "Adulto",
                "Descripcion": "Gato negro elegante, independiente y muy observador.",
                "FotoPath": "imagenes/gatos/adultos/imagen4.png"
            },
            {
                "Nombre": "Mishi", "Tipo": "Gato", "Raza": "Atigrado", "Edad": "Cachorro",
                "Descripcion": "Curioso y travieso. Le encanta perseguir juguetes.",
                "FotoPath": "imagenes/gatos/cachorros/imagen5.png"
            },
            # --- AVES ---
            {
                "Nombre": "Blue", "Tipo": "Ave", "Raza": "Guacamayo", "Edad": "Joven",
                "Descripcion": "Loro colorido de gran personalidad. Sabe decir 'Hola'.",
                "FotoPath": "imagenes/aves/imagen6.png"
            },
            {
                "Nombre": "Piolín", "Tipo": "Ave", "Raza": "Canario", "Edad": "Adulto",
                "Descripcion": "Canta todas las mañanas. Muy alegre.",
                "FotoPath": "imagenes/aves/imagen7.png"
            },
            {
                "Nombre": "Kiwi", "Tipo": "Ave", "Raza": "Periquito", "Edad": "Joven",
                "Descripcion": "Pequeño y verde. Le gusta estar acompañado.",
                "FotoPath": "imagenes/aves/imagen8.png"
            },
            {
                "Nombre": "Coco", "Tipo": "Ave", "Raza": "Cacatúa", "Edad": "Adulto",
                "Descripcion": "Muy inteligente y cariñosa con su dueño.",
                "FotoPath": "imagenes/aves/imagen9.png"
            },
            {
                "Nombre": "Red", "Tipo": "Ave", "Raza": "Cardenal", "Edad": "Adulto",
                "Descripcion": "Plumaje rojo intenso. Un ave muy vistosa.",
                "FotoPath": "imagenes/aves/imagen10.png"
            },
            # --- PECES ---
            {
                "Nombre": "Nemo", "Tipo": "Pez", "Raza": "Payaso", "Edad": "Joven",
                "Descripcion": "Pez de agua salada, famoso y aventurero.",
                "FotoPath": "imagenes/peces/imagen11.png"
            },
            {
                "Nombre": "Dory", "Tipo": "Pez", "Raza": "Cirujano", "Edad": "Adulto",
                "Descripcion": "Pez azul muy amigable, aunque un poco olvidadiza.",
                "FotoPath": "imagenes/peces/imagen12.png"
            },
            {
                "Nombre": "Goldie", "Tipo": "Pez", "Raza": "Goldfish", "Edad": "Joven",
                "Descripcion": "Clásico pez dorado. Muy fácil de cuidar.",
                "FotoPath": "imagenes/peces/imagen13.png"
            },
            {
                "Nombre": "Burbuja", "Tipo": "Pez", "Raza": "Betta", "Edad": "Adulto",
                "Descripcion": "Pez luchador de colores brillantes. Solitario.",
                "FotoPath": "imagenes/peces/imagen14.png"
            }
        ]
    ...
    def init_db(self):
        """Inicializar la conexión a SQL Server y crear tablas si no existen"""
        try:
            ...
            # Tabla adopcion
            cursor.execute("""
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='adopcion' AND xtype='U')
            ...
            # Tabla veterinarias
            cursor.execute("""
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='veterinarias' AND xtype='U')
            ...
            # Tabla blacklist_vet
            cursor.execute("""
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='blacklist_vet' AND xtype='U')
            ...
            # Tabla blacklist_maltrato
            cursor.execute("""
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='blacklist_maltrato' AND xtype='U')
            ...
            # Tabla mascotas_perdidas
            cursor.execute("""
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='mascotas_perdidas' AND xtype='U')
    ...
    def load_db(self):
        ...
        try:
            ...
            self.db_adopcion = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
            
            # Cargar veterinarias
            cursor.execute("SELECT * FROM veterinarias")
            self.db_veterinarias = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
            
            # Cargar blacklist_vet
            cursor.execute("SELECT * FROM blacklist_vet")
            self.db_blacklist_vet = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
            
            # Cargar blacklist_maltrato
            cursor.execute("SELECT * FROM blacklist_maltrato")
            self.db_blacklist_maltrato = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
            
            # Cargar mascotas perdidas
            cursor.execute("SELECT * FROM mascotas_perdidas")
            self.db_mascotas_perdidas = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
            
            # Cargar votos
            cursor.execute("SELECT * FROM votos")
            self.db_votos = [dict(zip([column[0] for column in cursor.description], row)) for row in cursor.fetchall()]
    ...
    def insert_adopcion(self, item):
        """Insertar un nuevo registro en la tabla de adopción"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO adopcion (Nombre, Tipo, Raza, Edad, ColorHex, Descripcion, FotoPath) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (item.get("Nombre"), item.get("Tipo"), item.get("Raza"), item.get("Edad"), 
         item.get("ColorHex"), item.get("Descripcion"), item.get("FotoPath")))
        self.conn.commit()

    def insert_veterinaria(self, vet):
        """Insertar una nueva veterinaria"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO veterinarias (Nombre, Direccion, Telefono, Descripcion, Servicios, FotoPath, Latitud, Longitud, PromedioEstrellas) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (vet.get("Nombre"), vet.get("Direccion"), vet.get("Telefono"), vet.get("Descripcion"), 
         vet.get("Servicios"), vet.get("FotoPath"), vet.get("Latitud"), vet.get("Longitud"), vet.get("PromedioEstrellas")))
        vet["ID"] = cursor.execute("SELECT SCOPE_IDENTITY()").fetchone()[0]  # Obtener el ID insertado
        self.conn.commit()

    def insert_blacklist_vet(self, item):
        """Insertar un nuevo reporte de veterinaria en la blacklist"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO blacklist_vet (NombreVeterinaria, Motivo, Latitud, Longitud, FechaReporte) 
            VALUES (?, ?, ?, ?, ?)
        """,
        (item.get("NombreVeterinaria"), item.get("Motivo"), item.get("Latitud"), 
         item.get("Longitud"), str(item.get("FechaReporte"))))
        self.conn.commit()

    def insert_blacklist_maltrato(self, item):
        """Insertar un nuevo reporte de maltrato"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO blacklist_maltrato (Nombre, Descripcion, FotoPath, Latitud, Longitud, FechaReporte) 
            VALUES (?, ?, ?, ?, ?, ?)
        """,
        (item.get("Nombre"), item.get("Descripcion"), item.get("FotoPath"), 
         item.get("Latitud"), item.get("Longitud"), str(item.get("FechaReporte"))))
        self.conn.commit()

    def insert_mascota_perdida(self, item):
        """Insertar una nueva mascota perdida"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO mascotas_perdidas (Nombre, Tipo, Raza, Edad, Descripcion, Contacto, Latitud, Longitud, FotoPath, Estado, FechaPerdido) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (item.get("Nombre"), item.get("Tipo"), item.get("Raza"), item.get("Edad"), 
         item.get("Descripcion"), item.get("Contacto"), item.get("Latitud"), item.get("Longitud"), 
         item.get("FotoPath"), item.get("Estado"), str(item.get("FechaPerdido"))))
        self.conn.commit()

    def insert_voto(self, voto):
        """Insertar un nuevo voto y actualizar promedio"""
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT INTO votos (VetID, Estrellas, Comentario) 
            VALUES (?, ?, ?)
        """,
        (voto.get("VetID"), voto.get("Estrellas"), voto.get("Comentario")))
        
        # Actualizar promedio de estrellas
        cursor.execute("SELECT AVG(CAST(Estrellas AS FLOAT)) as avg FROM votos WHERE VetID=?", (voto.get("VetID"),))
        r = cursor.fetchone()
        avg = r[0] if r and r[0] is not None else voto.get("Estrellas")
        cursor.execute("UPDATE veterinarias SET PromedioEstrellas=? WHERE ID=?", (avg, voto.get("VetID")))
        self.conn.commit()

    def cerrar_conexion(self):
        """Cerrar la conexión a la base de datos de forma segura"""
        if hasattr(self, 'conn') and self.conn:
            self.conn.commit()  # Asegurar que todos los cambios se guarden
            self.conn.close()

    def eliminar_mascota_perdida(self, mascota_id):
        """Eliminar una mascota perdida de la base de datos y de la lista en memoria"""
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM mascotas_perdidas WHERE id=?", (mascota_id,))
        self.conn.commit()
    ...
    # ==========================================
    # FUNCIONALIDAD SMS MANAGER
    # ==========================================
    def enviar_sms(self, numero_destino, mensaje):
        """Enviar un mensaje de texto SMS usando SMSManager"""
        try:
            # Preparar los datos para la API de SMSManager
            payload = {
                'api_key': SMSMANAGER_CONFIG['api_key'],
                'to': numero_destino,
                'message': mensaje,
                'sender_id': SMSMANAGER_CONFIG['sender_id']
            }
            
            # Llamar a la API de SMSManager
            response = requests.post(SMSMANAGER_CONFIG['api_url'], data=payload)
            
            if response.status_code == 200:
                print(f"SMS enviado exitosamente a {numero_destino}")
                return True
            else:
    ...
    def crear_imagen(self, path, size=(100, 100), fallback_color="#CCCCCC"):
        ruta_real = None
        if path:
            # Verificar si la ruta ya es absoluta o relativa a assets/uploads
            if os.path.exists(path):
                ruta_real = path
            else:
                # Probar diferentes posibles ubicaciones de la imagen
                posibles = [
                    path,  # Ruta tal como se guardó
                    os.path.join(os.getcwd(), path),
                    os.path.join("assets", path),
                    os.path.join("assets/uploads", path),
                    os.path.join("..", "assets", "uploads", os.path.basename(path))  # En caso de que la ruta incluya parte del path original
                ]
                for p in posibles:
        ...
        if ruta_real:
            try:
                ...
                print(f"Error al cargar imagen {ruta_real}: {e}")  # Para depuración
                pass
    ...
    def limpiar_frame(self):
        # Unbind any potentially lingering events before destroying widgets
        for widget in self.main_frame.winfo_children():
            # Try to unbind mousewheel events to prevent errors when widgets are destroyed
            try:
                widget.unbind("<MouseWheel>")
            except:
                pass  # Widget doesn't have this binding
            try:
    ...
    def crear_scroll_canvas(self, parent):
        ...
        # Bind mousewheel to the frame contained in the canvas to ensure events are captured
        frame.bind("<MouseWheel>", _on_mousewheel)  # Windows
        frame.bind("<Button-4>", lambda e: canvas.yview_scroll(-1, "units"))  # Linux
        frame.bind("<Button-5>", lambda e: canvas.yview_scroll(1, "units"))   # Linux

        # Make sure the frame can receive focus so mousewheel events are properly captured
        frame.bind("<Enter>", lambda e: canvas.focus_set())

        return frame

    # ==========================================
    # MENU PRINCIPAL (LOGO MÁS GRANDE)
    # ==========================================
    ...
    def mostrar_adopcion(self):
        ...
        tk.Label(self.f_edad, text="Edad:", font=("Arial", 11, "bold"), bg=COLOR_BG_MAIN, fg=COLOR_TEXT).pack(side="left", padx=(0, 10))
    ...
    def cargar_grid_adopcion(self, event=None):
        ...
        if not mascotas:
            tk.Label(self.scroll_frame_adopcion, text="No hay mascotas disponibles en esta categoría.",
                     font=("Arial", 14), fg="gray", bg=COLOR_BG_MAIN).pack(pady=50)
            return
        ...
        for m in mascotas:
            ...
            tk.Label(cf, text=m.get('Raza', 'Mestizo'), font=("Arial", 10), fg="#888", bg="white").pack(pady=(0, 10))
    ...
    def ver_detalle_mascota(self, mascota):
        ...
        for t in [mascota.get('Raza'), mascota.get('Tipo'), mascota['Edad']]:
            tk.Label(tags, text=f"• {t}", font=("Arial", 11, "bold"), bg="#FFE0B2", fg=COLOR_ORANGE_FILTER, padx=10, pady=5).pack(side="left", padx=5)

        tk.Label(info, text="Energía: Media/Alta ⚡", font=("Arial", 11, "bold"), fg=COLOR_PRIMARY, bg="white").pack(anchor="w", pady=(20, 5))
    ...
    def mostrar_form_inline(self, parent, mascota):
        ...
        def enviar():
            ...
            self.mostrar_exito(mascota, e_n.get(), e_t.get())
        tk.Button(f, text="ENVIAR SOLICITUD ➤", bg=COLOR_ACCENT, fg="white", font=FONT_BUTTON, width=25, pady=12, command=enviar).pack()
        parent.update_idletasks()

    def mostrar_exito(self, mascota, usuario, telefono_usuario):
        # Enviar SMS de confirmación usando SMSManager
        mensaje_sms = f"¡Hola {usuario}! Tu solicitud de adopción para {mascota['Nombre']} ha sido recibida. ¡Pronto te contactaremos!"
        self.enviar_sms(telefono_usuario, mensaje_sms)
    ...
    def form_adopcion(self):
        ...
        crear_campo("Edad Aproximada (ej: 2 meses):", "Edad")
        ...
        def publicar():
            if not entries["Nombre"].get() or not cb_tipo.get() or not self.ruta_imagen_temporal:
                messagebox.showwarning("Faltan datos", "Por favor completa el nombre, tipo y sube una foto.")
                return
            try:
                ...
                try:
                    self.insert_adopcion(self.db_adopcion[-1])
                except Exception:
                    pass
                messagebox.showinfo("¡Listo!", "La mascota ha sido publicada correctamente.")
                self.mostrar_adopcion()
            except Exception as e:
    ...
    def votar_vet(self, vet):
        ...
        def enviar():
            ...
            try:
                ...
                cursor = self.conn.cursor()
                cursor.execute("SELECT PromedioEstrellas as p FROM veterinarias WHERE ID=?", (vet['ID'],)); r=cursor.fetchone()
                vet['PromedioEstrellas'] = r[0] if r and r[0] is not None else vet['PromedioEstrellas']
            except Exception:
                pass
    ...
    def form_bl_vet(self):
        ...
        mv.add_left_click_map_command(on_click)
        def guardar():
            if not e_n.get() or not t_m.get("1.0", "end-1c") or not self.temp_vet_lat: messagebox.showwarning("!", "Completa nombre, motivo y ubicación."); return
        ...
        def guardar():
            ...
            try:
                ...
                nuevo_reporte = {
                    ...
                    "Descripcion": e_d.get(), 
                    "Contacto": e_c.get(), 
                    "Latitud": self.temp_lat, 
                    "Longitud": self.temp_lon, 
                    "FotoPath": dest, 
                    "Estado": tipo, 
                    "FechaPerdido": datetime.now()
                }
                ...
                # Enviar SMS de alerta usando SMSManager si es una mascota perdida
                if tipo == "Perdido":
                    mensaje_sms = f"¡Alerta! Se ha reportado una mascota perdida: {e_n.get()}, {cb_t.get()}. Tipo: {tipo}. Contacto: {e_c.get()}"
                    self.enviar_sms(e_c.get(), mensaje_sms)
    ...
    def cargar_lista_perdidos(self):
        ...
        for m in data:
            ...
            # Información adicional: Tipo, Edad, Raza
            ...
            # Botón eliminar (solo visible si hay id en el registro)
    ...
    def eliminar_perdido_confirmar(self, mascota):
        """Mostrar diálogo de confirmación antes de eliminar una mascota perdida"""
    ...
    def ver_mapa_perdido(self, m):
        ...
        panel = tk.Frame(map_c, bg="white", width=300, padx=20, pady=20); panel.pack(side="left", fill="y")
        img = self.crear_imagen(m.get('FotoPath'), size=(200, 200)); tk.Label(panel, image=img, bg="white").pack(pady=20)
        tk.Label(panel, text=m['Nombre'], font=("Helvetica", 24, "bold"), bg="white").pack()
    ...
    def form_bl_abuser(self):
        ...
        def guardar_denuncia():
            ...
            try:
                ...
                # Enviar SMS de alerta de maltrato usando SMSManager
                mensaje_sms = f"Alerta de maltrato animal reportado cerca de su área. Nombre: {nombre}"
                # Enviar a números relevantes configurados en el sistema
                # En una implementación real, se enviaría a números de autoridades o voluntarios
                self.enviar_sms("1234567890", mensaje_sms)  # Número de ejemplo
