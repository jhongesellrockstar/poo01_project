...
class PetApp(tk.Tk):
    ...
    def init_db(self):
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS adopcion (
            ...
            Nombre TEXT, Tipo TEXT, Raza TEXT, Edad TEXT, ColorHex TEXT, Descripcion TEXT, FotoPath TEXT
        )""")
    ...
    def insert_adopcion(self, item):
        c = self.conn.cursor()
        c.execute("INSERT INTO adopcion (Nombre, Tipo, Raza, Edad, ColorHex, Descripcion, FotoPath) VALUES (?,?,?,?,?,?,?)",
                  (item.get("Nombre"), item.get("Tipo"), item.get("Raza"), item.get("Edad"), item.get("ColorHex"), item.get("Descripcion"), item.get("FotoPath")))
        self.conn.commit()
    ...
    def insert_voto(self, voto):
        ...
        # actualizar promedio simple: tomar medias de votos en tabla
